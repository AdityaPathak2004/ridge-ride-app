...
export function epoch(day,minute){const target=Date.parse(day+'T00:00:00Z')+minute*60000;let guess=target;for(let i=0;i<4;i++){const p=localParts(new Date(guess));const actual=Date.parse(p.date+'T00:00:00Z')+p.minute*60000+p.second*1000;guess+=target-actual;}return guess;}
